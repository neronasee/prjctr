...
docker compose up -d mysql_master mysql_slave_1 mysql_slave_2 
...
until docker exec mysql_slave_1 sh -c 'export MYSQL_PWD=111; mysql -u root -e ";"'
do
...
until docker exec mysql_slave_2 sh -c 'export MYSQL_PWD=111; mysql -u root -e ";"'
do
